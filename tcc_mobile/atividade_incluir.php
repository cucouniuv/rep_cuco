...
// Arquivo 1
if (!empty($_FILES['documento_frente']['name'])) {
	...
	if (array_search($extensao, $_UP['extensoes']) === false) {
		$return_arr["check"] = 0;
		$return_arr["message"] = '*Por favor, envie arquivos com as seguintes extensões: jpg, tif ou pdf(documento frente)';
		echo json_encode($return_arr);
		exit ;
	}
	// Faz a verificação do tamanho do arquivo
		...
		$return_arr["check"] = 0;
		$return_arr["message"] = '*O arquivo enviado é muito grande, envie arquivos de até 2Mb (documento frente)';
		echo json_encode($return_arr);
		exit ;
	}
	// O arquivo passou em todas as verificações, hora de tentar movê-lo para a pasta
		...
		if (move_uploaded_file($_FILES['documento_frente']['tmp_name'], $_UP['pasta'] . $nome_arquivo1)) {
			...
			//print_r("Upload efetuado com sucesso!5");
		} else {
			$return_arr["check"] = 0;
...
// Arquivo 2
if (!empty($_FILES['documento_verso']['name'])) {
	...
	if (array_search($extensao, $_UP['extensoes']) === false) {
		$return_arr["check"] = 0;
		$return_arr["message"] = '*Por favor, envie arquivos com as seguintes extensões: jpg, tif ou pdf (Documento Verso)';
		echo json_encode($return_arr);
		exit ;
	}
	// Faz a verificação do tamanho do arquivo
		...
		$return_arr["check"] = 0;
		$return_arr["message"] = '*O arquivo enviado é muito grande, envie arquivos de até 2Mb (Documento Verso)';
		echo json_encode($return_arr);
		exit ;
	}
	// O arquivo passou em todas as verificações, hora de tentar movê-lo para a pasta
		...
		if (move_uploaded_file($_FILES['documento_verso']['tmp_name'], $_UP['pasta'] . $nome_arquivo2)) {
			...
			$return_arr["check"] = 0;
			$return_arr["message"] = '*Falha ao realizar upload do segundo arquivo (Documento Verso)';
			echo json_encode($return_arr);
			exit ;
		}
