				...
				<ul data-role="listview" data-divider-theme="b" data-inset="false" data-filter="true" data-filter-placeholder="Digite seu filtro aqui..." data-split-icon="delete" data-autodividers="true">
					...
					if ($lista_turma) {
						while ($row = mysql_fetch_array($lista_turma)) {
							...
							echo '<a href="administrador_turma_editar.php?id_turma=' . $row['id_turma'] . '" data-transition="slide">';
							echo '<h3>' . $row['nome'] . '</h3>';
							...
							echo '<a href="turma_excluir.php?codigo=' . $row['id_turma'] . '" data-transition="slide" onclick="return confirmaExclusao();" "></a>';
							echo '</a></li>';
						}
					}
					?>
